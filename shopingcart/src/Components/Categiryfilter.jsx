...
const Categiryfilter = () => {
  ...
  return (
    ...
    <div className='flex flex-wrap gap-3 border border-gray-800 pb-6 '>
      ...
      {availableCategories.map((category) => <button key={category} className={`px-5 py-2 text-sm font-bold rounded-full transition duration-200 shadow-md text-white ${selectedcategory === category ? 'bg-orange-600 shadow-orange-800/50' : 'bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-orange-400 border border-gray-700'}`}>
        {category}
      </button>)}
    </div>
    </>
